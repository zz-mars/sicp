...
(define (list-elem-i lst i)
 (cond ((null? lst) nil)
  ((= i 1) (car lst))
  (else (list-elem-i (cdr lst) (- i 1)))))

(define x (list 1 2 3 (list 4 5) 6))
(display "========== list-elem-i =========")
(newline)
(display (list-elem-i x 1))
(newline)
(display (list-elem-i x 2))
(newline)
(display (list-elem-i x 3))
(newline)
(display (list-elem-i x 4))
(newline)
(display (list-elem-i x 5))
(newline)
; safe?
(define g (list 1 3 5 2 4 6 8 7))
(define (safe? i guess)
 (define (safe-iter j)
  (if (= j i) (= 0 0)
   (let ((elem-i (list-elem-i guess i))
		 (elem-j (list-elem-i guess j)))
	(if (or (= elem-j elem-i)
		 (= (abs (- elem-i elem-j)) (- i j)))
	 (= 1 0)
	 (safe-iter (+ j 1))))))
 (safe-iter 1))
(define (safe-test i lst)
 (cond ((safe? i lst)
		(display i)
		(display " ---------> safe")
		(newline))
...
;  (define (queens-col k)
;	; Is the i'th element in guess safe
;	(define (safe? i guess)
;	  (define (safe-iter j)
;		(if (= j i) (= 0 0)
;		  (let ((elem-i (list-elem-i guess i))
...
;  Much faster implementation by interchanging the two map
(define (queens board-size)
  (define (queens-col k)
	; Is the i'th element in guess safe
	(define (safe? i guess)
	  ...
	  (safe-iter 1))
	(if (= k 0) (list nil)
	  (list-filter (flatmap (queens-col (- k 1))
					(lambda (already-placed)
					 (list-map (enumerate-interval 1 board-size)
					  (lambda (new-row)
					   (define (adjoin-new-row al k nr)
						(append al (list nr)))
					   (adjoin-new-row already-placed k new-row)))))
